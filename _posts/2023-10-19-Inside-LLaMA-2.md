...
Jargon warning!  This post gets very technical, as I'm trying to cover a huge amount of ground.  I'll unpack and expand on things in later posts.  It's also a work in progress.  Comments or suggestions are welcome!  For longer messages, you can reach me at lawray.ai@gmail.com.
...
1. The model is performing analogue computations, so **it is important to understand the effective “noise” that disrupts the fidelity of encoded information.**  There are clear noise issues with the fidelity of information read by the input, output, and middle dictionaries, but one can’t tell if this is noise in the model or just an issue with the dictionary.
   ...
   In practice the registries are <a href = "/docs/Llama-2/input-vs-output.html" target = "_blank" rel = "noreferrer noopener">not very orthogonal at all (see SVD figure)</a>.  The tradeoff the model seems to have accepted is that each registry can encode words with high specificity, but contains just a few intelligible words and has a high noise floor. My sense from expanded versions of Fig. 5-6 is that the words devolve into senselessness beneath a critical amplitude of F ~ 0.07. Some of this could be due to drift in the underlying dictionaries, as they don't need remain constant across the tranformer stack. But F ~ 0.07 is roughly consistent with what one would expect if you approximate words from other dictionaries as having random normally distributed vector elements [satisying 1 > 4096*0.5*(1-erf(F*sqrt(4096/2))].
