...
This trend in layer-to-layer similarity of the state vectors is consistent with what one expects to see as **information is loaded** into the state vectors.  I'll take a moment to play this out, as it's a nice window into analogue information storage.  Each state vector begins as the representation of a single token.  As a toy model, we can imagine each layer of the network as adding a several new words to the state vector, while preserving the overall amplitude of each word.  This means that the initial state of a token vector would be v_0 = v_token0, and the first layer output would include a couple more tokens (v_1 = v_token0 + v_token1 + v_token2), the second layer output would encode \~5 tokens (v_2 = v_token0 + v_token1 + v_token2 + v_token3 + v_token4), and so on.  Normalizing the state vectors for appropriate comparison gives a next-layer inner product of <v_i\|v_i+1> = sqrt((2i+1)/(2i+3)) between layers 'i' and 'i+1'.  This rapidly converges towards 1, with initial values of (<v_0\|v_1>) 0.58 and (<v_1\|v_2>) 0.85 that are very similar to the average trend in Fig. 12 (blue curve). 
